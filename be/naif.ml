...
let rec find_index p l =
  match l with
  | [] -> None
  | t::q -> if (p t) then Some 0 else 
      match find_index p q with
      | None -> None
      | Some i -> Some (i+1)

let%test _ = find_index (fun i -> i mod 2 = 0) [1;3;4;5;6;7] = Some 2
let%test _ = find_index (fun i -> i mod 2 = 0) [1;3;5;7] = None
let%test _ = find_index (fun c -> c > 'd') ['a';'b';'c';'d';'e';'f';'g'] = Some 4
let%test _ = find_index (fun c -> c > 'r') ['a';'b';'c';'d';'e';'f';'g'] = None
...
let%test _ = try let _ = encoder_lettre t9_map '&' in false with _ -> true  
...
let%test _ = try let _ = encoder_lettre stupide_map '&' in false with _ -> true       
*)


(* TODO *)
let creer_liste = fun _ -> assert false
...
let decoder_lettre = fun _ -> assert false
...
let decoder_mot = fun _ -> assert false
